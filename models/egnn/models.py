...
from models.egnn.egnn_new import EGNN, GNN
from models.equivariant_diffusion.utils import remove_mean, remove_mean_with_mask
import numpy as np
...
class EGNN_dynamics_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
        ...
        if mode == 'egnn_dynamics':
            self.egnn = EGNN(
                ...
                virtual_token_dim=virtual_token_dim)
    ...
    def _forward(self, t, xh, node_mask, edge_mask, context, virtual_token=None):
        ...
        # --- 新增：处理 virtual_token 的维度 ---
        if virtual_token is not None:
            if virtual_token.dim() == 2:
                virtual_token = virtual_token.unsqueeze(1).expand(bs, n_nodes, -1).reshape(bs * n_nodes, -1)
            else:
                virtual_token = virtual_token.reshape(bs * n_nodes, -1)
        # ------------------------------------
