...
import numpy as np
from models.egnn.models import EGNN_dynamics_QM9, EGNN_encoder_QM9, EGNN_decoder_QM9

from models.equivariant_diffusion.en_diffusion import EnVariationalDiffusion, EnHierarchicalVAE, EnLatentDiffusion
...
def get_autoencoder(args, device, dataset_info, dataloader_train=None):
    histogram = dataset_info['n_nodes']
    in_node_nf = len(dataset_info['atom_decoder']) + int(args.include_charges)
    nodes_dist = DistributionNodes(histogram)
...
def get_latent_diffusion(args, device, dataset_info, dataloader_train=None, finetune_args=None):
    ...
    net_dynamics = EGNN_dynamics_QM9(
        ...
        inv_sublayers=args.inv_sublayers, sin_embedding=args.sin_embedding,
        normalization_factor=args.normalization_factor, aggregation_method=args.aggregation_method,
        virtual_token_dim=finetune_args.virtual_token_dim if finetune_args is not None else None)
