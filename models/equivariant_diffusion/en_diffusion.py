from models.equivariant_diffusion import utils
import numpy as np
import math
import torch
from tqdm import tqdm
from models.egnn import models
...
def gaussian_KL_for_dimension(q_mu, q_sigma, p_mu, p_sigma, d):
    ...
    assert len(q_sigma.size()) == 1, f"q_sigma 维度错误! 形状为: {q_sigma.size()}, 预期为 1D (Batch_size,)"
    assert len(p_sigma.size()) == 1, f"p_sigma 维度错误! 形状为: {p_sigma.size()}, 预期为 1D (Batch_size,)"
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def phi(self, x, t, node_mask, edge_mask, context, virtual_token=None):
        net_out = self.dynamics._forward(t, x, node_mask, edge_mask, context, virtual_token=virtual_token)
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always, virtual_token=None):
        ...
        net_out = self.phi(z_t, t, node_mask, edge_mask, context, virtual_token=virtual_token)
        ...
        if t0_always:
            ...
            net_out = self.phi(z_0, t_zeros, node_mask, edge_mask, context, virtual_token=virtual_token)
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, virtual_token=None):
        """
        ...
        if self.training:
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, context, t0_always=False, virtual_token=virtual_token)
        else:
            # Less variance in the estimator, costs two forward passes.
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, context, t0_always=True, virtual_token=virtual_token)
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, virtual_token=None):
        ...
        if self.training:
            ...
            loss_ld, loss_dict = self.compute_loss(z_x, z_h, node_mask, edge_mask, context, t0_always=False, virtual_token=virtual_token)
        else:
            # Less variance in the estimator, costs two forward passes.
            loss_ld, loss_dict = self.compute_loss(z_x, z_h, node_mask, edge_mask, context, t0_always=True, virtual_token=virtual_token)
