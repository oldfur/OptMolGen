...
from models.equivariant_diffusion.utils import assert_correctly_masked
import torch
import pickle
...
def main():
    ...
    parser.add_argument('--model_path', type=str,
                        default="outputs/drugs_latent2",
                        help='Specify model path')
    parser.add_argument('--n_samples', type=int, default=2,
                        help='number of samples to generate')
    parser.add_argument('--n_tries', type=int, default=4,
    ...
    flow, nodes_dist, _ = get_latent_diffusion(args, device, dataset_info)
    flow.to(device)
    ...
    sample_only_stable_different_sizes_and_save(
        args, eval_args, device, flow, nodes_dist,
        dataset_info=dataset_info, n_samples=eval_args.n_samples, n_tries=eval_args.n_tries)
